...
public class LoadTelNrColumn {

	public void loadTelNrColumn(TableView<TableModel> tableView, TableColumn<TableModel, String> firstNameColumn, TableColumn<TableModel, String> lastNameColumn,
		...
		for(int i = 0; i < aboutDatabase.getRowsCount(); i++) {
			searchModelObservableList.add(new TableModel(dataManager.getFirstNames().get(i), dataManager.getLastNames().get(i), dataManager.getBirthdays().get(i), 
					dataManager.getEmails().get(i), dataManager.getLoadedTelNrs().get(i), dataManager.getDegrees().get(i), dataManager.getMeetingDays().get(i), dataManager.getGenders().get(i)));			
		}
