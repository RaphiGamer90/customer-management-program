...
public interface LoadTelNrColumn {

	default public void loadtelnrcolumn(TableView<TableModel> tableView, TableColumn<TableModel, String> firstNameColumn, TableColumn<TableModel, String> lastNameColumn,
		...
		for(int i = 0; i < aboutDatabase.getRowsCount(); i++) {
			searchModelObservableList.add(new TableModel(dataManager.getUnloadedFirstNames().get(i), dataManager.getUnloadedLastNames().get(i), dataManager.getUnloadedBirthdays().get(i), 
					dataManager.getUnloadedEmails().get(i), dataManager.getLoadedTelNrs().get(i), dataManager.getUnloadedDegrees().get(i), dataManager.getUnloadedMeetingDays().get(i), dataManager.getUnloadedGenders().get(i)));			
		}
