...
public interface LoadLastNameColumn {

	default public void loadlastnameColumn(TableView<TableModel> tableView, TableColumn<TableModel, String> firstNameColumn, TableColumn<TableModel, String> lastNameColumn,
		...
		for(int i = 0; i < aboutDatabase.getRowsCount(); i++) {
			searchModelObservableList.add(new TableModel(dataManager.getUnloadedFirstNames().get(i), dataManager.getLoadedLastNames().get(i), dataManager.getUnloadedBirthdays().get(i), 
		...
		System.out.println("Es wurde LastNameColumn geladen!");
	}
